...
- **Shared client runtime** (`internal/edge/`) — The physical client transport (TCP/TLS accept, UDP socket, `connection.Conn` lifecycle, Version banner) and the client ingress ownership router (`ClientIngressRouter`: every inbound control message is classified Edge/Split/Core once, in `edge.OwnershipOf`) live here and are composed identically by every mode; nothing Edge-specific is copied into business packages. Edge-owned protocol helpers (Reject flush-then-close, CryptSetup resync, server-list ping replies) are shared from `internal/edge/protocol.go`.
- **Runtime modes** — `internal/app` is the composition root: it validates the mode's config, then constructs exactly that mode's dependency graph. `standalone` = Core + Local Edge in one process (`server.Server`). `core` = the standalone graph plus `cluster.RemoteEdgeServer` bound to the real session registry between `Prepare` and `Run`. `edge` = `edge.EdgeRuntime` + `edge.RemoteCoreClient` over the shared client runtime — **no database is opened**, no user/channel/ACL/ban/identity state, no Core voice router, no REST plane. The edge-core network protocol (mTLS, heartbeats, auth relay, voice relay) is the next phase; until then a remote-core call fails closed with `ErrCoreProtocolNotImplemented` and authentication ends in a Reject — an Edge never substitutes a local Core. `[distributed].mode = "edge"` additionally requires `core_address` and `edge_id`.
...
- **UDP sender identification** — Incoming UDP packets are mapped to client sessions using an address-based cache (populated after the first packet from each client). The cached session's TLS-negotiated `CryptState` is used for decryption directly, avoiding trial decryption on every packet. On cache miss (first packet or NAT rebinding), the server falls back to trial decryption in priority order: secure, legacy, lite. Trial decrypt failures from the same host but a different port (e.g. probe vs voice) are logged at Debug level to reduce noise.
